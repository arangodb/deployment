all: DigitalOcean_ArangoDB_Cluster.sh GoogleComputeEngine_ArangoDB_Cluster.sh Azure_ArangoDB_Cluster.sh AmazonWebServices_ArangoDB_Cluster.sh GoogleComputeEngine_Mesos_Cluster.sh vCloud_ArangoDB_Cluster.sh
...
vCloud_ArangoDB_Cluster.sh: Makefile platformVCLOUD/VMWareVCloudAir_ArangoDB_Cluster.sh Docker/ArangoDBClusterWithDocker.sh
	echo "#!/bin/bash" > $@
	cat Docker/ArangoDBClusterWithDocker.sh >> $@
	cat platformVCLOUD/VMWareVCloudAir_ArangoDB_Cluster.sh >> $@
	chmod 755 $@

clean:
	rm -f DigitalOcean_ArangoDB_Cluster.sh GoogleComputeEngine_ArangoDB_Cluster.sh Azure_ArangoDB_Cluster.sh AmazonWebServices_ArangoDB_Cluster.sh GoogleComputeEngine_Mesos_Cluster vCloud_ArangoDB_Cluster.sh
