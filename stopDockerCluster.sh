...
# This stops an ArangoDB cluster by stopping the docker containers
#
# Prerequisites:
...
stop_dbserver () {
    i=$1
    echo Stopping DBserver on ${SERVERS_EXTERNAL[$i]}:$PORT_DBSERVER

    $SSH_CMD ${SSH_USER}@${SERVERS_EXTERNAL[$i]} $SSH_SUFFIX \
             docker stop dbserver$PORT_DBSERVER >/dev/null
}

stop_coordinator () {
    i=$1
    echo Stopping Coordinator on ${SERVERS_EXTERNAL[$i]}:$PORT_COORDINATOR

    $SSH_CMD ${SSH_USER}@${SERVERS_EXTERNAL[$i]} $SSH_SUFFIX \
             docker stop coordinator$PORT_COORDINATOR >/dev/null
}
...
$SSH_CMD ${SSH_USER}@${SERVERS_EXTERNAL[0]} $SSH_SUFFIX  docker stop discovery > /dev/null &

echo Stopping agency

$SSH_CMD ${SSH_USER}@${SERVERS_EXTERNAL[0]} $SSH_SUFFIX docker stop agency >/dev/null &
